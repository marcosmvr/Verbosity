...
export async function generateQuizQuestionsBatch(amount) {
    try {
        const chatResponse = await client.chat.complete({
            ...
            messages: [
                {
                    role: 'system',
                    content: 'Você é um professor de inglês didático, criando exercícios de múltipla escolha para brasileiros. As questões devem focar em gramática ou vocabulário. Cada pergunta deve ter uma única resposta correta entre quatro opções (1 correta e 3 incorretas).',
                },
                {
                    role: 'user',
        ...
        if (typeof rawContent === 'string') {
            try {
                const batchResponse = JSON.parse(rawContent);
                ...
                console.error('Erro ao fazer parse do JSON recebido da API da IA:', parseError);
                console.error('Conteúdo recebido (possível JSON inválido):', rawContent);
                throw new Error('Resposta da IA não é um JSON válido ou não está no formato de lote esperado.');
            }
        }
        else {
            console.error('Conteúdo inesperado recebido da API da IA. Não é uma string.');
            throw new Error('Formato de resposta inesperado da API da IA.');
        }
    }
    catch (error) {
        console.error('Erro ao gerar perguntas em lote com Mistral:', error.message || error);
        throw new Error('Não foi possível gerar as perguntas no momento. Verifique sua chave de API ou conexão.');
    }
}
